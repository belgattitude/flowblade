...
export const createRandomTable = (params?: Params): Table => {
    ...
    if (prefix !== '' && !isDuckdbValidIdentifier(prefix)) {
        throw new Error('Invalid table prefix. Only alphanumeric characters and underscores are allowed.')
    }
    const nanoid = customAlphabet('1234567890abcdefghijklmnopqrstuvw', 15)
    const tableName = [prefix.trim() === '' ? false : prefix,  nanoid()].filter(Boolean).join('_');
